...
object DisplayUtil {
    ...
    /**
     * 获取App真实高度，包含可能存在的虚拟导航栏，未显示的刘海区域，状态栏等  兼容multi-window和小窗模式
     */
    fun getAppRealHeight(window: Window): Int {

        return if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.R) {
            val result = window.windowManager.currentWindowMetrics.bounds.height()
            result
        }else{
    ...
    fun isPad(context: Context):Boolean=(context.resources.configuration.screenLayout and Configuration.SCREENLAYOUT_SIZE_MASK) >= Configuration.SCREENLAYOUT_SIZE_LARGE
    ...
    fun getNavigationBarHeight(context: Context, window: Window): Int {
        ...
        if (manufacturer.toLowerCase(Locale.ROOT).contains("samsung")) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.P && Build.VERSION.SDK_INT < Build.VERSION_CODES.Q) {
                ...
                val navigationBarHeight = ViewCompat.getRootWindowInsets(window.decorView)?.getInsets(
    ...
    fun isNavBarVisible(context: Context, window: Window): Boolean {
        ...
        viewGroup?.let {
            ...
            if (!isVisible) {
                val navigationBarView: View? = it.findViewById(com.xtree.live.R.id.immersion_navigation_bar_view)
